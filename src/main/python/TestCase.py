...
"""
Number of iterations for which we verify the conformance against the expected results.
...
when using an approach that uses Sentinel's "no data" instead of IEEE 754 NaN values.
...
"""
NaN bit pattern for missing data. A value may be missing for different reasons, which are identified
by different NaN values. This test uses the following values, in order of precedence. For example,
...
then the result will be considered missing for the `LAND` reason.
  ...
  - Missing because the pixel is on land (assuming that the data are for some oceanographic phenomenon).
...
def floatToRawIntBits(f):
    ...
    This is needed for inspecting which NaN value was obtained.
    """
    s = struct.pack('>f', f)
...
"""
The directory where to look for binary files. These files are generated by the Java test.
...
This value should be zero during the first few iterations and only become non-zero
after the calculation has drifted. Note that the latter case is not an indication
that NaN does not work because identical mismatches happen with the "no data" approach too.
"""
...
def load_coordinates():
    ...
    This is for avoiding the need to check for bounds before bilinear interpolation.
    """
    bytes = array.array('B')
...
def print_statistics():
    ...
    uses a chaotic algorithm in order to test the effect of optimizations enabled
...
def compute_and_compare():
    """
    Reads the raster, interpolates multiple values, and compares the results against the expected values.
    ...
    for it in range(0, NUM_VERIFIED_ITERATIONS):
        ...
        for i in range(0, NUM_INTERPOLATION_POINTS):
            ...
            #
            # Apply bilinear interpolation. Contrary to the `TestNodata` case, we compute
            # this interpolation unconditionally without checking if the data are valid.
            # This is an arbitrary choice, we could have made the two sets of code more similar.
            # We do this for illustrating this flexibility, and for showing that we can
            ...
            # (single precision) to 64 bit floating point numbers (double precision) before doing the
            ...
            #   2) NaN payloads are sorted with higher values for the reason of having precedence.
            #
            # The combination of these two facts allows us to simply check for the maximum value,
            # using signed integer comparisons, even if we have a mix of "no data" and real values.
            ...
            if math.isnan(result):
                ...
                # This step is not needed in an application using NaN. This test is doing this
